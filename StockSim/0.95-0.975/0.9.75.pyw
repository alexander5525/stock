# Stock Sim Version 0.9.75
# Author: alexander5525
# Finished: 11/25/2023
# Description: Stock strategy analyzer that allows strategies to be built in a scratch like interface, thoroughly analyzed and reviewed and be tested on s&p 500 live data
...
def meanRelativeRise(stock, spot, n): # get mean exponential rise from spot in percent
    mu = 0
    if len(stock)-1 >= spot + n: # make sure there are at least 16 more samples
        weights = exp_values(n)
        for s in range(n):
...
def meanAbsoluteRise(stock, spot, n): # get mean exponential rise from spot in absolute terms (-1, 1)
    # also called positivity index
    mu = 0
    if len(stock)-1 >= spot + n: # make sure there are at least 16 more samples
        weights = exp_values(n)
        for s in range(n):
            # perc = (stock[spot+s][3]/stock[spot][3]-1)*100 # get percentage
            # mu += perc*weights[s] # add weighted values
            mu += int(stock[spot+s][3]>stock[spot][3])*weights[s] # add 1 per value greater than spot value
        c = (sum(weights)-weights[0])/2
        mu = mu / c - 1 # this will make mu=1 the best trade and mu = -1 the worst trade
        # mu /= 16 # get mean
    return mu

def changeIndex(stock, spot, n): # number from -1 to inf (theoretically) that tells how the price is going to go the next n spots
    mu = 0
    ...
    if len(stock)-1 >= spot + n: # make sure there are at least 16 more samples
        weights = exp_values(n)
        for s in range(1, n):
            if stock[spot+s][3] > stock[spot][3]: posind += 1
            else: posind -= 1
            p = stock[spot+s][3] - stock[spot+s-1][3]
            p *= weights[s]
            if p > 0: posneg[0] += p
            else: posneg[1] -= p
        if posneg[0] == 0: return float("inf")
...
class SmallView(QtWidgets.QGraphicsView): # small view window for condition creator
    ...
    def initScene(self): # same as newScene for gui
        ...
        if self.spot != -1: # focus on the spot and the 60 around the spot
            # if self.spot < 30: bot = 0
            # else: bot = self.spot - 30 
            # if self.spot + 30 > len(self.candles): top = self.candles - 1
            # else: top = self.spot + 30
            # self.rangex = (bot, top)
        ...
        if self.spot == -1: 
            ...
            self.rangey = (mi, ma)
        # else: last = self.candles[self.spot][3]
        # elif self.spot + 30 > len(self.candles): last = self.candles[-3][3]
        # else: last = self.candles[self.spot+28][3]
...
class IndButton(QtWidgets.QPushButton): # Indicator Button
    ...
    def setExFn1(self, fn, name): # extra option
        self.exFn1 = fn
        self.exName1 = name

    def setExFn2(self, fn, name): # extra option
        self.exFn2 = fn
        self.exName2 = name
    ...
    def contextMenuEvent(self, event):
        ...
        if self.exFn1 is not None:
            act = menu.addAction(self.exName1)
            act.triggered.connect(lambda: self.exFn1(self.idd))
        if self.exFn2 is not None:
            act = menu.addAction(self.exName2)
            act.triggered.connect(lambda: self.exFn2(self.idd))
        ...
        if theme == "dark": menu.setStyleSheet("color: white;")
...
class AttributeLineEdit(QtWidgets.QLineEdit): # line edit with type to determine what can be typed into this
    def __init__(self, parent, typ:type):
        super().__init__(parent)
        self.typ = typ
...
class StratList(QtWidgets.QListWidget): # custom list for strategies; makes it easier to get id of clicked thing
    def __init__(self, parent=None):
        ...
        # self.coData = [] # additional data for keeping track of items
...
class OperationList(QtWidgets.QListWidget):
    def __init__(self, parent):
        ...
        self.sortBy = None # this is the category that future new entries will be sorted by
        self.reversed = False # whether to reverse sorted list
        self.attrSort = [] # how the attributes are sorted i.e. ["Ticker", "Time", "Exit Percentage"]
...
class OperationListItem(QtWidgets.QListWidgetItem):
    def __init__(self, dicts, parent=None, displist=[], showConds=False):
        super().__init__(parent)
        self.dicct = dicts
        if "No." in list(self.dicct.keys()): st = str(self.dicct["No."]) + " "
        else: st = ""
        if displist == []:
...
class BackThread(): # thread that runs the backtests in the background
    def __init__(self, fn, increment, rawind, cons, indx):
        ...
        self.process = None # the process that is started when it is started
        self.queue = None # where the values get passed to
        self.doReview = False # whether backthread is part of the review or not
        self.revind = -1 # index of strategy in a review
    
    def start(self): # start the thread
        self.process = multiprocessing.Process(target=self.fn, args=(self.ind, self, self.queue, self.doReview)) 
        self.process.start()
...
class DragBox(QtWidgets.QComboBox): # combobox that allows dragging
    def __init__(self, parent=None, delfn=None, edfn=None):
        ...
        self.rnFn = None
        self.doDrag = True
        self.boxType = "V" # v for variable, i for indicator, c for condition
    ...
    def updateToolTip(self): # tooltip
        if len(self.vars) == 0 or self.boxType == "C": return
...
class Logic(): # class for all of the logic things needed in the gui
    def __init__(self):
        ...
        self.stats = Stats() # stats that will be displayed in the bottom left
    ...
    def getConditionData(self, variables=[], conid=None, stock=-1, review=False, dates=None):
        ...
        if dates is None: # if no dates are given
            for s in self.systems:
                if s.rawind == stock: break
            dates = s.timeaxis # get dates from selected system
        if type(variables[0]) == str: # if a filename was entered
            spec = util.spec_from_file_location("user_module", variables[0])
            userModule = util.module_from_spec(spec)
            spec.loader.exec_module(userModule)
            fn = getattr(userModule, "data")
            data = fn(raw[stock], dates, variables[1])
        ...
        for t in range(len(toplay)):
            ...
            deps.reverse()
            for d in deps: 
                if d[:2] not in calc: calc.append(d[:2])
        ...
        data = []
        if review:
            vardicts = {} # dictionaries that will store all of the variable data at each spot
            dictorder = []
            for c in calc[1]:
                if c[0] == "i": # indicator
                    indic = varsorted[3][find("i", c[1])]
                    name = indic.indName
                    for a in indic.args:
                        if "%" not in str(a): name += " " + str(a) # not variable
                        else: 
                            ty = a[1]
                            idd = int(a.split("%")[1].split(ty)[1])
                            if ty == "v": search = varsorted[0]
                            elif ty == "e": search = varsorted[1]
                            if len(search) != 0:
                                for v in search:
                                    if v.id == idd: break
                                if ty == "v": name += " " + v.var
                    ...
                    for v in varsorted[0]:
                        if v.indName == indic.indName and v.args == indic.args: # for every variable of indicator
                            vardicts[name + " " + v.var] = []
                            dictorder.append(("v", v.id))
                else:
                    temp = ["v", "e", "x"]
                    t = temp.index(c[0])
                    vardicts[varsorted[t][find(c[0], c[1])].name] = []
                    dictorder.append((c[0], c[1]))
        for t in range(len(raw[stock])):
            ...
            for c in calc[1]:
                if c[0] == "i": # indicator
                    ...
                    if var.type != "Variable": # skip variable because it has already been calculated
                        ...
                        if var.type == "Dynamic Near": args.append(0.005*raw[stock][t][3])
                        out = expression(var.type, args)
                        var.val = out
                if review:
                    if c[0] == "i":
                        for v in varsorted[0]:
                            if v.indName == var.indName and v.args == var.args:
                                if ("v", v.id) in dictorder: 
                                    if type(v.val) == list: vardicts[list(vardicts.keys())[dictorder.index(("v", v.id))]].append(v.val[t])
                                    else: vardicts[list(vardicts.keys())[dictorder.index(("v", v.id))]].append(v.val)
                    else:
                        temp = ["v", "e", "x"]
                        v = varsorted[temp.index(c[0])][find(c[0], c[1])]
                        #print(list(vardicts.keys())[dictorder.index((c[0], c[1]))], t)
                        if type(v.val) == list: vardicts[list(vardicts.keys())[dictorder.index((c[0], c[1]))]].append(v.val[t])
                        else: vardicts[list(vardicts.keys())[dictorder.index((c[0], c[1]))]].append(v.val)
                if c in calc[0]:
            ...
            if ended: 
                if review:
                    for key in list(vardicts.keys()):
                        if len(vardicts[key]) == t: vardicts[key].append(None)
                data.append(False)
            else:
                ands = []
        ...
        if review: return data, vardicts
        return data
    ...
    def getData(self, ind): # calculate data for conditions
        if len(self.conditions[ind]["deps"]) == 0: # if indicator condition
            if len(self.conditions[ind]["data"]) == 0: # check if data has not been calculated
                dat = self.getConditionData(conid=self.conditions[ind]["ID"])
    ...
    def getRisk(self, ind=-1, risk=[], getRecalc=False, once=False, stock=-1, review=False): # return risk values and also whether it needs to be recalculated
        ...
        if getRecalc: # check whether risk has to be recalculated at every step
            if risk[0][1] is None or risk[0][2] is None: 
                return True # if expressions are used in the final activation step | then risk has to be recaluclated all the time
            used = []
            for r in risk[0]:
                if type(r) != list:
                    if "%" in str(r):
        ...
        varsorted = [[], [], [], []] # variables, equations, expressions, indicators
        for v in risk[1]:
            if type(v) == VariableEquation: varsorted[1].append(v)
            elif type(v) == VariableExpression: varsorted[2].append(v)
            else:
                varsorted[0].append(v)
                # also make support indicator and append to third list
                i = 0
                while i < len(varsorted[3]):
                    if varsorted[3][i].indName == v.indName and varsorted[3][i].args == v.args: break # check whether indicator already exists
                    i += 1
                if i == len(varsorted[3]): # if indicator wasnt found
                    varsorted[3].append(IndicatorVariable(v.indName, v.args, idd=i)) # add indicator
        ...
        for r in risk[0]:
            if type(r) != list:
                ...
                for rr in r:
                    if "%" in str(rr):
                        used.append(rr)
        if len(used) == 0 and not None in risk[0]: # if no variables to calculate and no rxit expressions
            if review: return [risk[0]], {}
        ...
        if len(risk[2]) == 0:
            ...
            if risk[0][1] is None or risk[0][2] is None: # if expressions are used to get exits
                # get every top layer expression and add them to toplay
                # toplay = [] # top layer of expressions
                for ex in varsorted[2]: toplay.append(("x", ex.id))

                for ex in varsorted[2]:
                    for a in ex.args:
                        if "%x" in str(a):
                            if ("x", int(a.split("%x")[1])) in toplay: toplay.remove(("x", int(a.split("%x")[1]))) # remove all expressions used to calculate another
        ...
        for time in range(len(raw[stock])):
            ...
            for c in calc:
                if c[0] == "i": # indicator
                    ...
                    if var.type == "Dynamic Near": args.append(0.005*raw[stock][time][3])
                    out = expression(var.type, args)
                    var.val = out
                if review:
                    if c[0] == "i":
                        for v in varsorted[0]:
                            if v.indName == var.indName and v.args == var.args:
                                if ("v", v.id) in dictorder: 
                                    if type(v.val) == list: vardicts[list(vardicts.keys())[dictorder.index(("v", v.id))]].append(v.val[time])
                                    else: vardicts[list(vardicts.keys())[dictorder.index(("v", v.id))]].append(v.val)
                    else:
                        temp = ["v", "e", "x"]
                        v = varsorted[temp.index(c[0])][find(c[0], c[1])]
                        #print(list(vardicts.keys())[dictorder.index((c[0], c[1]))], t)
                        if type(v.val) == list: vardicts[list(vardicts.keys())[dictorder.index((c[0], c[1]))]].append(v.val[time])
                        else: vardicts[list(vardicts.keys())[dictorder.index((c[0], c[1]))]].append(v.val)
            
            final = True
            for t in risk[2]:
                if t[0] == "x": # only get expressions from top layer
                    final = final and varsorted[2][find("x", t[1])].val
                    if not final: break # if one expression is false, dont bother with the rest
        ...
        if review: return vals, vardicts
        return vals
            ...
            if not error:
                ...
                for op in operations:
                    if op.type == "Stop Limit":
                        if raw[op.ind][time][3] <= op.stop: # if stop loss is reached
                            ...
                            money += op.sell(time)
                            poplist.append(operations.index(op))
                    elif op.type == "Trailing Stop": # trailing stop
                        if raw[op.ind][time][3]*(1-op.trai) > op.stopprice: op.stopprice = raw[op.ind][time][3]*(1-op.trai) # if price went up, follow price
                            ...
                            money += op.sell(time)
                            poplist.append(operations.index(op))
                    elif risk[time][2] and op.time != time: # if no order type found; expressions are used instead; if these are true
                        money += op.sell(time)
                        poplist.append(operations.index(op)) # sell all current operations
                poplist.reverse() # reverse list, so that later indexes are removed first
                sold = False
                for p in poplist: # remove finished operations
                    if operations[p].type == "Trailing Stop":
                    ...
                    else: # stop limit or expressions
                ...
                if self.strategies[ind]["data"][time]: # if strategy here is true
                    if money >= fees+amnt*raw[stock][time][3]: 
                        ...
                        elif stoptype == "Stop Limit": # stop limit
                            operations.append(Operation(stock, "Stop Limit", amnt, time, stlo=stopvals[0], tapr=stopvals[1], fee=fees))
                        else:
                            operations.append(Operation(stock, "Market Order", amnt, time, fee=fees))
    ...
    def backthread(self, ind, back, queue, review=False): # function run by each backthread
        global raw
        ...
        if len(back.prefs) != 0: # if live data
            if back.prefs[2] == "":
                ...
                except: # if no internet or link not found
                    return
                df = table[0]
        ...
        def getData(ind): # redefine getdata to deglobalize variables
            ...
            if len(self.conditions[cind]["deps"]) == 0: # if indicator condition
                if len(back.condata[ind]) == 0: # check if data has not been calculated
                    if review: # for review get all of the variable values
                        dat, varvals = self.getConditionData(conid=self.conditions[cind]["ID"], stock=back.rawind, review=True, dates=dates) # change what stock is used
                        convals[self.conditions[cind]["ID"]] = varvals
                    else:
                        dat = self.getConditionData(conid=self.conditions[cind]["ID"], stock=back.rawind, dates=dates) # change what stock is used
        ...
        while back.rawpoint < len(avstocks) -1:
            ...
            if not uselive: raw[back.rawind] = read(avstocks[back.rawpoint])
                ...
                raw[back.rawind], dates = stock_data(avstocks[back.rawpoint], period=back.prefs[0], interval=back.prefs[1])
            ...
            if len(raw[back.rawind]) != 0: # not nothing has been loaded
                ...
                if review: allops = [] # all operations summarrized in dictionaries
                time = 0
                ...
                riskvals = None
                if review:
                    risk, riskvals = self.getRisk(risk=back.risk, once=static, stock=stock, review=True) # will return either a static len == 1 list or a len == len(stock)
                else:
                    risk = self.getRisk(risk=back.risk, once=static, stock=stock)
                startmoney = risk[0][0] # balance
                stoptype = risk[0][1] # what kind of operation will be started
                stopvals = risk[0][2]
                amnt = int(ceil(risk[0][3]/raw[stock][0][3]))
                ...
                while time < timeframe: # timeframe for stock simulation
                    ...
                    if not error:
                        ...
                        for op in back.operations:
                            # if review: poplen = len(poplist) # store old len
                            if op.type == "Stop Limit":
                                if raw[op.ind][time][3] <= op.stop: # if stop loss is reached
                                    money += op.sell(time)
                                ...
                                money += op.sell(time)
                                poplist.append(back.operations.index(op)) # sell all current operations
                        poplist.reverse() # reverse list, so that later indexes are removed first
                        sold = False
                        for p in poplist: # remove finished operations
                            if back.operations[p].type == "Trailing Stop": # append exitprc using trailing stop, time and buy price
                            ...
                            if review: 
                                if uselive: 
                                    back.operations[p].reviewVals["Exit Time"] = [d.to_pydatetime().replace(tzinfo=None) for d in dates][time]
                                back.operations[p].reviewVals["Exit Spot"] = time
                                back.operations[p].reviewVals["Exit Price"] = raw[stock][time][3]
                                back.operations[p].reviewVals["Exit Percentage"] = back.entexs[2][-1][1]
                                valley = rateSurrounding(raw[stock], time, changeIndex, 16)[0]
                                back.operations[-1].reviewVals["Exit PR"] = valley # exit position rating
                                back.operations[-1].reviewVals["Exit New µ"] = changeIndex(raw[stock], time, 16)
                                for v in riskvals:
                                    back.operations[-1].reviewVals["Ex. " + v] = riskvals[v][time] # save exit strategy values too as "ex. " + name
                                allops.append(back.operations[p].reviewVals)
                        ...
                        if back.data[time]: # if strategy here is true
                            if money >= fees+amnt*raw[stock][time][3]: # if buyable
                                ...
                                elif stoptype == "Stop Limit":
                                    back.operations.append(Operation(stock, "Stop Limit", amnt, time, stlo=stopvals[0], tapr=stopvals[1], fee=fees))
                                else:
                                    back.operations.append(Operation(stock, "Market Order", amnt, time, fee=fees))
                                if review: # get every value needed to know how operation happened
                                    back.operations[-1].reviewVals["Ticker"] = avstocks[back.rawpoint]
                                    if uselive: 
                                        back.operations[-1].reviewVals["Entry Time"] = [d.to_pydatetime().replace(tzinfo=None) for d in dates][time]
                                        back.operations[-1].reviewVals["Period"] = back.prefs[0]
                                        back.operations[-1].reviewVals["Interval"] = back.prefs[1]
                                    else:
                                        back.operations[-1].reviewVals["Period"] = "2y"
                                        back.operations[-1].reviewVals["Interval"] = "1h"
                                    back.operations[-1].reviewVals["Entry Spot"] = time
                                    back.operations[-1].reviewVals["Entry Price"] = raw[stock][time][3]
                                    back.operations[-1].reviewVals["Balance"] = money + fees + amnt * raw[stock][time][3]
                                    back.operations[-1].reviewVals["Amount"] = amnt
                                    back.operations[-1].reviewVals["Entry Rel. µ"] = meanRelativeRise(raw[stock], time, 16)
                                    back.operations[-1].reviewVals["Entry Abs. µ"] = meanAbsoluteRise(raw[stock], time, 16)
                                    back.operations[-1].reviewVals["Entry New µ"] = changeIndex(raw[stock], time, 16)
                                    peak = rateSurrounding(raw[stock], time, changeIndex, 16)[0]
                                    back.operations[-1].reviewVals["Entry PR"] = peak # entry position rating
                                    for c in back.conds:
                                        # ind = self.find("c", c[1])
                                        ind = back.conds.index(c)
                                        cind = self.find("c", c[1])
                                        cond = self.conditions[cind]
                                        back.operations[-1].reviewVals[cond["name"]] = back.condata[ind][time]
                                        if len(cond["deps"]) == 0: # indicator condition
                                            for key in list(convals[cond["ID"]].keys()):
                                                back.operations[-1].reviewVals[key] = convals[cond["ID"]][key][time]
            ...
            if review: 
                if len(allops) != 0:
                    if uselive: 
                        sortas = ["Ticker", "Entry Time", "Period", "Interval", "Entry Spot", "Entry Price", "Exit Spot", "Exit Time", "Exit Price", "Exit Percentage"]
                    else:
                        sortas = ["Ticker", "Entry Spot", "Period", "Interval", "Entry Price", "Exit Spot", "Exit Price", "Exit Percentage"]
                    if back.revind != -1: sortas = ["No."] + sortas
                    sortas = sortas + ["Entry PR", "Exit PR", "Exit New µ"]
                    for key in list(allops[0].keys()):
                        if key not in sortas: sortas.append(key)
                    sortops = []
                    for a in allops:
                        if back.revind != -1: a["No."] = back.revind # set strategy number (if multiple are being calculated)
                        sortops.append(dict(sorted(a.items(), key=lambda item: sortas.index(item[0])))) # sort the list afterwards
                    queue.put(sortops)
                else:
                    queue.put(allops)
            else: queue.put((outs, back.ind))
        ...
        if review: 
            if len(allops) != 0:
                ...
                queue.put(allops)
        else: queue.put(([self.stats], back.ind))
        # print("Finished in ")
...
class GUI(QtWidgets.QMainWindow):
    ...
    def create_widgets(self): # init function
        ...
        # preferences menu
        self.prefs = {}
        self.prefs["Calculate strategies on all available data"] = True
        self.prefs["Recalculate strategies after editing conditions"] = True
        self.prefs["Calculate strategies on live data"] = True
        self.prefs["Ask to name variables"] = False
        self.prefs["When debugging skip to next marked stock"] = True
        ...
        act = edit.addAction("Templates...")
        act.triggered.connect(self.templateDialog)
        ...
        act = view.addAction("Review Strategies...")
        act.triggered.connect(self.reviewDialog)
        view.addSeparator()
    ...
    def deleteTab(self, index): # delete tab at cursor
        ...
        if backrem:
            ...
            if debug: # if debug is also open
                ...
                logic.currentSystem = 0 # reset index here as well
        ...
        if changeTo is not None: # if the current tab was deleted and a new one has to be loaded
            self.focus.placed = False
            self.moved = False
            ...
            logic.rawind = logic.systems[changeTo].rawind
        ...
        if event.modifiers().name == "ControlModifier": # execute multi marking
            if typ == "c":
                ...
                if len(logic.conditions[ind]["data"]) == 0: # if data hasn't yet been loaded
                    logic.getConditionData(conid=idd) # dont check for complex because it can only be indicator
    ...
    def saveConfig(self): # take all memory variables and store them in config.xml
        ...
        with open(os.path.join(os.path.dirname(__file__), "config.xml"), 'wb') as f:
    ...
    def templateDialog(self): # edit a template file
        dbox = QtWidgets.QDialog(self)
        dbox.setWindowTitle("Edit Templates...")
        dbox.setFixedSize(300, 150)
        QtWidgets.QLabel("Select a template", dbox).move(10, 10)
        dlist = QtWidgets.QListWidget(dbox)
        dlist.setGeometry(10, 35, 200, 100)

        keeplist = [] # list of dictionary keys to keep after save button was pressed
        filename = ""
        data_dict = []
        ok = False
        ...
        def tempOpen():
            ...
            nonlocal keeplist
            keeplist = list(data_dict.keys())

        btn = QtWidgets.QPushButton("Open", dbox)
        btn.setGeometry(136, 7, 75, 25)
        btn.clicked.connect(tempOpen)

        def remove():
            nonlocal keeplist
            item = dlist.currentItem()
            dlist.takeItem(dlist.row(item)) # take item out of list
            keeplist.remove(item.text())

        btn = QtWidgets.QPushButton("Remove", dbox)
        btn.setGeometry(225, 34, 60, 25)
        btn.clicked.connect(remove)

        def save():
            nonlocal data_dict
            nonlocal ok
            poplist = set(data_dict.keys()) - set(keeplist) # make the difference of the 2 lists as a set
            for p in poplist: del data_dict[p]
        ...
        def uSure():
            if not ok and filename != "":
                threading.Thread(target=lambda:playsound("Exclamation")).start()
                result = QtWidgets.QMessageBox.question(self, "Edit Templates...", f"Do you want to save changes to {filename}?", 
                QtWidgets.QMessageBox.StandardButton.Yes | QtWidgets.QMessageBox.StandardButton.No, QtWidgets.QMessageBox.StandardButton.No)
                if result != QtWidgets.QMessageBox.StandardButton.Yes: # only continue if yes is pressed
                    return
                save()
    ...
    def conditionCreatorLayout(self, first=False, ind=-1): # layout for condion creator
        ...
        if mode: # if advanced
            if modechanged: # if last mode was simplified
                if type(self.inputs[3][1]) == str and self.inputs[3][1] != "": # if a py path was entered
                    threading.Thread(target=lambda:playsound("Exclamation")).start()
                    result = QtWidgets.QMessageBox.question(self, "Condition Creator...", f"Do you want to stop using {self.inputs[3][1]}?", 
                    QtWidgets.QMessageBox.StandardButton.Yes | QtWidgets.QMessageBox.StandardButton.No, QtWidgets.QMessageBox.StandardButton.No)
                    if result != QtWidgets.QMessageBox.StandardButton.Yes: # only continue if yes is pressed
                        return
            ...
            self.inputs[0][4].addItems(["Compare", "Combine", "Not", "Dynamic Near", "Near"])
            if len(olds) != 0:
            ...
            elif self.inputs[0][4].currentText() == "Near":
                QtWidgets.QLabel("Near", wid).move(10, 254)
                self.inputs[1][2].append(Slot(wid, self.dropBox))
                self.inputs[1][2][0].setGeometry(50, 254, 75, 22)
                self.inputs[1][2].append(Slot(wid, self.dropBox))
                self.inputs[1][2][1].setGeometry(130, 254, 75, 22)
                QtWidgets.QLabel("Dis.", wid).move(210, 254)
                self.inputs[1][2].append(Slot(wid, self.dropBox))
                self.inputs[1][2][2].setGeometry(237, 254, 73, 22)
        ...
        if mode:
            ...
            def fileDial():
                self.inputs[3][1] = QtWidgets.QFileDialog.getOpenFileName(self, "Open condition file...", "", "Python File (*.py)")[0] # get filename
                self.creatorNewLayout()

            btn = QtWidgets.QPushButton("Use External .py File", wid)
            btn.setGeometry(440, 360, 150, 23)
            btn.clicked.connect(fileDial)

            def how():
                st = "How to use a .py file as a condition:\n"
                st += "1. Write a method named data in the file\n"
                st += "2. Make data accept the values: stock and time\n"
                st += "3. Make sure time can also be [] and still function"
                st += "4. Return a list (len==len(stocks)) with boolean values\n"
                st += "5. Make sure your file has no main code\n"
                mbox = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Icon.Information, "Help", st, QtWidgets.QMessageBox.StandardButton.Ok, wid)
                mbox.exec()

            btn = QtWidgets.QPushButton("?", wid)
            btn.setGeometry(595, 360, 23, 23)
            btn.clicked.connect(how)

            if type(self.inputs[3][1]) == str and self.inputs[3][1] != "":
                QtWidgets.QLabel(f"Pressing OK will only use the contents of {self.inputs[3][1].split('/')[-1]} to calculate this condition.", wid).move(10, 330)
                spec = util.spec_from_file_location("user_module", self.inputs[3][1])
                userModule = util.module_from_spec(spec)
                spec.loader.exec_module(userModule)
                argsTypes = getattr(userModule, "argsTypes", {}) # get the arguments needed for the condition
                self.inputs[3][2] = []
                i = 0
                for a in argsTypes:
                    if argsTypes[a] == bool:
                        chk = QtWidgets.QCheckBox(str(a), wid)
                        chk.move(10+i*100, 200)
                        self.inputs[3][2].append(chk)
                    else:
                        QtWidgets.QLabel(a, wid).move(10+i*100, 185)
                        lin = AttributeLineEdit(wid, argsTypes[a])
                        lin.setGeometry(10+i*100, 200, 75, 22)
                        self.inputs[3][2].append(lin)
                    i += 1
    ...
    def getName(self, typ, extra=[]): # gets name for given variable using its type and args
        ...
        if typ == "e":
            ...
            if self.inputs[0][4].currentText() in ["Compare", "Combine"]:
                ...
                name += "Not "
                name += self.inputs[1][2][0].text()
            elif "Near" in self.inputs[0][4].currentText():
                name += "Near("
                name += self.inputs[1][2][0].text()
                if type(self.inputs[1][2][0].var) == IndicatorVariable:
                    if str(self.inputs[1][2][0].spotVar) != "-1": name += "(" + str(self.inputs[1][2][0].spotVar) + ")"
                ...
                if self.inputs[0][4].currentText() == "Near":
                    name += ", "
                    name += self.inputs[1][2][2].text()
                    if type(self.inputs[1][2][2].var) == IndicatorVariable:
                        if str(self.inputs[1][2][2].spotVar) != "-1": name += "(" + str(self.inputs[1][2][2].spotVar) + ")"
        ...
        elif typ == "rx": # risk expression
            if extra[0] in ["Compare", "Combine"]:
                name += extra[1].text()
                if type(extra[1].var) == IndicatorVariable:
                    if str(extra[1].spotVar) != "-1": name += "(" + str(extra[1].spotVar) + ")"
                name += extra[2].currentText()
    ...
    def addButton(self, what, edit=False): # different add buttons in conditon creator
        if what == "Indicator":
            ...
            if self.prefs["Ask to name variables"]:
                dbox = QtWidgets.QDialog(self.dialog)
    ...
    def creatorExecute(self, parent): # checks for errors and adds condition to conditions 
        ...
        if mode:
            ...
            varsorted = [[], [], [], []]
            if type(self.inputs[3][1]) == str and self.inputs[3][1] != "":
                # add an error check here to see whether a .py file is valid
                filename = self.inputs[3][1]
                for a in self.inputs[3][2]:
                    if type(a) == QtWidgets.QCheckBox:
                        args.append(a.isChecked())
                    else: # attribute line edit
                        try:
                            if a.typ != bool: args.append(a.typ(a.text()))
                            else: args.append(a.text().lower() in ["true", "1"])
                        except ValueError:
                            self.errormsg(f"{a.text()} is an invalid {a.typ}.")
                            return
                color = self.inputs[2][0].styleSheet().split(" ")[1][:-1] # only store color
            else:
        ...
        if filename is None:
            ...
            condict = {"ID":idd, "vars":[filename, args], "data":[], "color":color, "name":name, "filters":[], "deps":[]}
        if ind == -1: logic.conditions.append(condict)
        else: logic.conditions[ind] = condict 

        data = logic.getConditionData(conid=idd)
        ...
        if self.prefs["Recalculate strategies after editing conditions"]: #and not self.inputs[2][0].isChecked():
    ...
    def strategyDialog(self, idd=False): # dialog box for running strategies
        ...
        def riskDialog(): # dialog for risk management
            ...
            dbox3 = None # just to prevent the yellow marking
            ...
            def dropBox(box: Slot): # what happens when a variable is dropped in a slot
                ...
                elif box.pInd == 1: foc = dbox3.focusWidget()
            ...
            lab = QtWidgets.QLabel("Exit Strategy", dbox2)
            lab.move(10, 35)
            ...
            def displayOrder(): # makes label for order
                if self.inputs[3][1] is not None: # if regular order types are used
                    st = ""
                    st += self.inputs[3][1] + " "
                ...
                else: # if expressions are being used
                    labchange.setText("Exit Expression")
            ...
            def exitDialog():
                ...
                dbox3.setWindowTitle("Exit Strategy...")
                dbox3.setFixedSize(400, 200)
                QtWidgets.QLabel("Variables", dbox3).move(10, 10)
                varcopy = DragBox(dbox3)
                ...
                QtWidgets.QLabel("Equations", dbox3).move(10, 50)
                eqcopy = DragBox(dbox3)
                eqcopy.setGeometry(10, 66, 110, 22)
                its = [eqs.itemText(indx) for indx in range(eqs.count())]
                eqcopy.addItems(its)
                eqcopy.vars = eqs.vars
                QtWidgets.QLabel("Trailing Stop", dbox3).move(125, 26)
                QtWidgets.QLabel("Trailing in decimal", dbox3).move(195, 10)
                slot1 = Slot(dbox3, dropBox)
                slot1.pInd = 1
                slot1.setGeometry(195, 26, 75, 22)
                QtWidgets.QLabel("Stop Limit", dbox3).move(125, 66)
                QtWidgets.QLabel("Stop Price", dbox3).move(195, 50)
                QtWidgets.QLabel("Limit Price", dbox3).move(255, 50)
                slot2 = Slot(dbox3, dropBox)
                slot2.setGeometry(195, 66, 50, 22)
                slot2.pInd = 1
                slot3 = Slot(dbox3, dropBox)
                ...
                def choose(what): # checks for errors and stores the values
                    ...
                    elif what == "x": # expression
                        self.inputs[3][1] = None
                        self.inputs[3][2] = None
                    displayOrder()
                    dbox3.close()

                btn = QtWidgets.QPushButton("Choose", dbox3)
                btn.setGeometry(310, 26, 73, 23)
                btn.clicked.connect(lambda: choose("t"))
                
                btn = QtWidgets.QPushButton("Choose", dbox3)
                btn.setGeometry(310, 65, 73, 24)
                btn.clicked.connect(lambda: choose("sl"))

                btn = QtWidgets.QPushButton("Choose", dbox3)
                btn.setGeometry(310, 104, 73, 24)
                btn.clicked.connect(lambda: choose("x"))

                QtWidgets.QLabel("Expressions", dbox3).move(10, 89)
                expbox = DragBox(dbox3)
                expbox.setGeometry(10, 105, 110, 22)
                expbox.addItems(exs[0])
                expbox.vars = exs[1]
                QtWidgets.QLabel("Add an expression", dbox3).move(130, 89)
                cbox = QtWidgets.QComboBox(dbox3)
                cbox.setGeometry(130, 105, 110, 22)
                cbox.addItems(["Compare", "Combine", "Not", "Dynamic Near", "Near"])

                def addDialog(): # opens add expression dialog
                    nonlocal exs
                    typ = cbox.currentText()
                    self.dialog = QtWidgets.QDialog(dbox3)
                    self.dialog.setWindowTitle("Add " + typ + "...")
                    self.dialog.setFixedSize(360, 150)
                    
                    args = []

                    if typ in ["Compare", "Combine"]:
                        args.append(Slot(self.dialog, self.dropBox))
                        args[0].setGeometry(10, 10, 110, 22)
                        args.append(QtWidgets.QComboBox(self.dialog))
                        args[1].setGeometry(122, 10, 48, 22)
                        args.append(Slot(self.dialog, self.dropBox))
                        args[2].setGeometry(172, 10, 110, 22)
                        if typ == "Compare": args[1].addItems(["==", "<", "<=", ">", ">="])
                        else: 
                            args[1].addItems(["and", "or", "xor"])
                            args[0].requested = "E" 
                            args[0].setLocked(True)
                            args[2].requested = "E" 
                            args[2].setLocked(True)
                    elif typ == "Not":
                        QtWidgets.QLabel("not", self.dialog).move(10, 10)
                        args.append(Slot(self.dialog, self.dropBox))
                        args[0].move(50, 10)
                        args[0].requested = "E" 
                        args[0].setLocked(True)
                    elif typ == "Dynamic Near":
                        QtWidgets.QLabel("Near", self.dialog).move(10, 10)
                        args.append(Slot(self.dialog, self.dropBox))
                        args[0].move(50, 10)
                        args.append(Slot(self.dialog, self.dropBox))
                        args[1].move(175, 10)
                        ...
                        QtWidgets.QLabel("Near", self.dialog).move(10, 10)
                        args.append(Slot(self.dialog, self.dropBox))
                        args[0].setGeometry(50, 10, 75, 22)
                        args.append(Slot(self.dialog, self.dropBox))
                        args[1].setGeometry(130, 10, 75, 22)
                        QtWidgets.QLabel("Near", self.dialog).move(210, 10)
                        args.append(Slot(self.dialog, self.dropBox))
                        args[2].setGeometry(237, 10, 73, 22)
                    ...
                    eqcopy.addItems(its)
                    eqcopy.vars = eqs.vars
                    QtWidgets.QLabel("Expressions", self.dialog).move(240, 50)
                    excopy = DragBox(self.dialog)
                    excopy.setGeometry(240, 66, 110, 22)
                    its = [expbox.itemText(indx) for indx in range(expbox.count())]
                    excopy.addItems(its)
                    excopy.vars = expbox.vars
                    ...
                    def addExp():
                        ...
                        if self.prefs["Ask to name variables"]:
                            ...
                            btn.clicked.connect(dbox.close)
                            dbox.exec()
                            if tbox.text() == "": name = self.getName("rx", [typ]+args)
                            else: name = tbox.text()
                        else: name = self.getName("rx", [typ]+args)
                        ...
                        for inp in args:
                            if type(inp) != Slot: # cbox
                                ...
                                if inp.var is not None: # variable in slot
                                    ...
                                    elif type(inp.var) == VariableExpression: t = "x" # expression
                                    else: # list with multiple variables
                                        doList = True
                                    if not doList: # anything but a list
                                        ...
                                        for v in inp.var:
                                            ...
                                            elif type(v) == VariableExpression: t = "x" # expression
                                            st += t
                                            st += str(inp.var.id)
                                            if t == "v": # also include spot
                                                i += 1 # only advance i here because spots were only recorded from variables
                        ...
                        i = 0
                        idd = 0
                        while i < len(expbox.vars): # check if id is already in use
                            if expbox.vars[i].id == idd:
                                idd += 1
                                i = -1 # if id in use go up and restart process
                            i += 1

                        expbox.vars.append(VariableExpression(name, typ, argvals, idd))
                        expbox.addItem(name)
                        self.dialog.close()
                        self.dialog.close()
                    ...
                    btn.move(142, 120)
                    btn.clicked.connect(addExp)
                    
                    self.dialog.exec()
                
                add = QtWidgets.QPushButton("Add...", dbox3)
                add.setGeometry(250, 104, 50, 24)
                add.clicked.connect(addDialog)

                def onClose():
                    nonlocal exs
                    exs[0] = [expbox.itemText(indx) for indx in range(expbox.count())]
                    exs[1] = expbox.vars

                dbox3.finished.connect(onClose)

                dbox3.exec()

            btn.clicked.connect(exitDialog)
            ...
            exs = [[], []] # expressions [[names], [variables]]
            ...
            if len(old) != 0: # if risk is being edited
                ...
                for o in old[1]:
                    if type(o) == IndicatorVariable: 
                        vrs.addItem(o.var)
                        evs[1].append(o)
                    elif type(o) == VariableExpression:
                        exs[0].append(o.name)
                        exs[1].append(o)
                    else: 
                        eqs.addItem(o.name)
                ...
                if pbox.currentText()[-1] == "d": # day range
                    ...
                    if int(pbox.currentText()[:-1]) <= 2: # max 2 years
                        ...
                        if ibox.currentText() in avail1[:avail1.index("1d")]:
                            ...
                            return
            groups = [x.name.upper() for x in self.commons if type(x) == Group]
            if tbox.text().upper() in groups: # if a group was entered
                for c in self.commons:
                    if type(c) == Group:
                        if c.name.upper() == tbox.text().upper(): break
                tickers = deepcopy(c.items) # copy tickers
                st = ""
                for t in tickers:
                    st += t + ","
                st = st[:-1] # remove last comma
                tbox.setText(st) # set all tickers to comma seperated strategy text
    ...
    def strategyExecute(self, parent, indx=False): # ok button | adds strategy to list
        ...
        if self.inputs[2][1].isChecked(): # debug mode
            ...
            self.debugvar = [False, -1, -1, False]
        self.backtest(logic.find("s", idd))
        global sp500 
        if self.prefs["Calculate strategies on live data"]:
            ...
            sp500 = [] # unload for the future
        if self.prefs["Calculate strategies on all available data"] and not dont:
            self.multiback(idd)
    ...
    def calcStrategy(self, idd): # calculate a strategy given the id
        ...
        if self.prefs["Calculate strategies on live data"]:
            ...
            sp500 = [] # unload for the future
        if self.prefs["Calculate strategies on all available data"]:
            self.multiback(idd)
        self.tabs.addTab("Backtest")
        self.tabs.addTab("Exit Percentages")
        self.tabs.addTab("Benchmark Comparison")
    ...
    def multiback(self, idd): # will run multiple backtests of strategy
        ...
        inc = self.backSettings["increment"] # increment and number of processes
        # logic.backthreads = []
        logic.manyStats = inc*[None]

        for i in range(inc):
            proc = BackThread(logic.backthread, inc, len(raw), deepcopy(logic.strategies[logic.find("s", idd)]["conds"]), i) # make new process
            proc.calc = deepcopy(logic.strategies[logic.find("s", idd)]["calc"])
            proc.risk = deepcopy(logic.strategies[logic.find("s", idd)]["risk"])
            proc.queue = self.queue
            if self.prefs["Calculate strategies on live data"]: proc.prefs = deepcopy(logic.strategies[logic.find("s", idd)]["prefs"])
            procManager.processes[procManager.currentIndex()].processes.append(proc) # append to list of processes
            procManager.processes[procManager.currentIndex()].processes[-1].start() # start registered process
    ...
    def reviewDialog(self): # dialog box that lists details of strategies and can compare two strategies
        dbox = QtWidgets.QDialog(self)
        dbox.setFixedSize(600, 400)
        dbox.setWindowTitle("Detailed Strategy Review...")
        QtWidgets.QLabel("Select a strategy", dbox).move(10, 10)
        strats = QtWidgets.QComboBox(dbox)
        strats.setGeometry(10, 26, 150, 22)
        for strat in logic.strategies:
            strats.addItem(strat["name"])
        
        def dblClick(item : OperationListItem):
            tline.setText(item.dicct["Ticker"])
            ibox.setCurrentText(item.dicct["Interval"])
            pbox.setCurrentText(item.dicct["Period"])
            sline.setText(str(item.dicct["Exit Spot"]))
            if item.dicct["Exit Percentage"] < 0: col = "#55ff0000"
            else: col = "#5500ff00"
            view.rects = [[item.dicct["Entry Spot"], item.dicct["Entry Price"], item.dicct["Exit Spot"], item.dicct["Exit Price"], col]]
            showView()
        
        oList = OperationList(dbox)
        oList.setGeometry(10, 75, 235, 150)
        oList.itemDoubleClicked.connect(dblClick)
        oList.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        running = False
        numstocks = 0
        processed = 0
        last = [None, None] # last strategy processed
        smart = SmartStats() # list to display a maximum of 2 items (for 2 strategies)
        smartList= QtWidgets.QListWidget(dbox)
        smartList.setGeometry(9, 227, 235, 45)
        smartList.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        smartList.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)

        # def doSort(): # sort all values in olist by whatever is selected in sbox
        #     oList.sortedList(sbox.currentText())
        defaultsorts = ["Newest", "Ticker", "Entry Time", "Period", "Interval", "Entry Spot", "Entry Price", "Exit Spot", "Exit Price", "Exit Percentage"]
        sorts = deepcopy(defaultsorts)
        oList.attrSort = ["Ticker", "Entry Time", "Exit Percentage"] # what should be displayed by each list item
        sortby = ["Newest", False] # sort by what, whether to reverse order

        hassorted = [False, None] # first is always for main strategy, second is for additional strategy (None for not available)
        # hassorted = False # whether the program already has the sorting categories
        
        # sbox.currentIndexChanged.connect(doSort)
        ...
        def configure(): # dbox to configure
            ...
            sbox.addItems(sorts)
            sbox.setCurrentText(sortby[0])
            revcheck = QtWidgets.QCheckBox("Reverse", db)
            revcheck.move(185, 35)
            revcheck.setChecked(sortby[1])
            hcheck = QtWidgets.QCheckBox("Show Hrz. Scrollbar", db)
            hcheck.move(10, 160)
            if oList.horizontalScrollBarPolicy() != QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff: hcheck.setChecked(True)
            check = QtWidgets.QCheckBox("Show Condition Values", db)
            check.move(150, 160)
            check.setChecked(oList.showConditions)
            ...
            for s in sorts[1:]:
                ...
                if s in oList.attrSort: item.setCheckState(QtCore.Qt.CheckState.Checked)
                else: item.setCheckState(QtCore.Qt.CheckState.Unchecked)
                slist.addItem(item)
            def ok():
                nonlocal sortby
                if hcheck.isChecked(): oList.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAsNeeded)
                else: oList.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
                oList.showConditions = check.isChecked()
                sortby = [sbox.currentText(), revcheck.isChecked()]
                oList.attrSort = []
                for ind in range(slist.count()):
                    item = slist.item(ind)
                    if item.checkState() == QtCore.Qt.CheckState.Checked:
                        oList.attrSort.append(item.text())
                oList.sortedList(sortby)
                db.close()
            btn = QtWidgets.QPushButton("OK", db)
            btn.move(115, 190)
            btn.clicked.connect(ok)
        ...
        btn = QtWidgets.QPushButton("Configure List...", dbox)
        btn.setGeometry(10, 50, 100, 22)
        btn.clicked.connect(configure)

        prog = QtWidgets.QProgressBar(dbox)
        prog.setGeometry(179, 275, 100, 22)
        prog.setValue(0)
        ...
        def stop():
            ...
            if running:
                ...
                procManager.delist("review")
            
                self.threads[-1].quit()
                del self.threads[-1]
        
        queue = multiprocessing.Queue() # queue to use

        def getQueue(ec=0): # get everything out of the queue and display it properly 
            nonlocal processed
            nonlocal running
            # nonlocal hassorted
            # if queue updated | ec is the end counter
            if not queue.empty():
                ec = 0
                vals = queue.get() # list of dicts from the operations
                first = True # if the first iteration of vals
                if not hassorted[0] and len(vals) != 0:
                    keys = list(vals[0].keys())
                    if "No." in keys: # check whether two sort keys have to be gotten
                        if vals[0]["No."] == 1: # if first strategy
                            hassorted[0] = True
                            for k in keys:
                                if k not in sorts:
                                    sorts.append(k)
                    else: # else this check only has to be done once
                        hassorted[0] = True
                        for k in keys:
                            if k not in sorts:
                                sorts.append(k)
                if hassorted[1] == False and len(vals) != 0:
                    if vals[0]["No."] == 2: # if second strategy
                        hassorted[1] = True
                        for k in keys:
                            if k not in sorts:
                                sorts.append(k)
                #     sbox.clear()
                #     sbox.addItem("Newest") # standard sorting setting
                #     sorts = [str(x) for x in list(vals[0].keys())] # list of values to sort by
                #     sbox.addItems(sorts)
                for v in vals:
                    if "No." in list(v.keys()): key = v["No."]
                    else: key = "Main"
                    if key not in list(smart.entexs.keys()): smart.entexs[key] = {"n+": 0, "n-": 0, "n": 0, "s+p": 0, "s-p": 0}
                    if first: smart.entexs[key]["n"] += 1 # add one stock because one is always put out per queue update
                    if v["Exit Percentage"] > 0: 
                        smart.entexs[key]["n+"] += 1
                        smart.entexs[key]["s+p"] += v["Exit Percentage"]
                    else: 
                        smart.entexs[key]["n-"] += 1
                        smart.entexs[key]["s-p"] += v["Exit Percentage"]
                    oli = OperationListItem(v)
                    # oli.setClickFn(lambda: print("ye"))
                    oList.addItem(oli)
                    oList.operations.append(v)
                    first = False
                oList.sortedList(sortby)
                smart.generate()
                smartList.clear()
                for i in range(len(smart.names)):
                    if len(smart.names) == 1: key = "Main"
                    else: key = i+1
                    if key in list(smart.entexs.keys()):
                        st = f"{smart.names[i]}: Success: {smart.succs[key]:2.1%}, s/f: {smart.sfs[key]:.2f}"
                        smartList.addItem(st)
                    # i += 1
                prog.setValue(int((smart.processed/numstocks)*100))
            elif ec == 2000 or not running: 
            ...
            QtCore.QTimer.singleShot(5, lambda:getQueue(ec))

        def run(): # runs the strategy in a backprocess and shows the values from queue
            nonlocal running
            nonlocal numstocks
            nonlocal last
            nonlocal processed
            ...
            if running:
                self.errormsg("Review has already been started.\nProcess has to be stopped first to start again.")
                return
            strat = logic.strategies[strats.currentIndex()]

            if combox.currentText() == "None": # if nothing should be compared
                pass
            else:
                thing = combox.vars[combox.currentIndex()]
                if thing[0][0] == "c": # if a condition was selected
                    if obox.currentIndex() == 0:
                        self.errormsg("Select an operator to compare with a condition.")
                        return
                    newstrat = deepcopy(strat)
                    operator = obox.currentText()
                    if operator == "Append":
                        if thing[0] == "cc": 
                            self.errormsg("Complex conditions can't be added.")
                            return
                        if thing in newstrat["conds"]:
                            result = QtWidgets.QMessageBox.question(self, "Continue?", "This condition is already in this strategy.\nAdd it anyways?", 
                            QtWidgets.QMessageBox.StandardButton.Yes | QtWidgets.QMessageBox.StandardButton.No, QtWidgets.QMessageBox.StandardButton.No)
                            if result != QtWidgets.QMessageBox.StandardButton.Yes: # only continue if yes is pressed
                                return
                        newstrat["conds"].append(thing)
                        newstrat["calc"].append(True)
                    elif operator == "Remove (only if used)":
                        if thing not in newstrat["conds"]:
                            self.errormsg("Can't remove something that's not there.")
                            return
                        rems = [thing]
                        deps = []
                        def tree(con):
                            nonlocal deps
                            for c in newstrat["conds"]:
                                cond = logic.find("c", c[1])
                                if con in cond["deps"]: # if condition is used complexly; 
                                    deps.append(c)
                                    tree(c)
                        rems += deps
                        for r in rems:
                            newstrat["calc"].pop(newstrat["conds"].index(r))
                            newstrat["conds"].remove(r) # remove all dependencies + self
                        
                        # if thing[0] == "ci":
                        #     isComplex = False
                        #     isNegated = False
                        #     for d in deps:
                        #         cond = logic.find("c", d[1])
                        #         if cond["deps"][1] == "not": isNegated = True
                        #         else: isComplex = True
                        #     if isComplex:
                        #         for d in deps:
                        #             cond = logic.find("c", d[1])
                    ...
                    elif operator == "Replace with...":
                        dia = QtWidgets.QDialog(dbox)
                        dia.setWindowTitle("Replace...")
                        dia.setFixedSize(300, 100)
                        QtWidgets.QLabel("Replace which condition with " + combox.currentText() + "?", dia).move(10, 10)
                        conditions = QtWidgets.QComboBox(dia)
                        conditions.move(10, 26)
                        for c in strat["conds"]:
                            cond = logic.conditions[logic.find("c", c[1])]
                            conditions.addItem(cond["name"])
                        ...
                        def pushok():
                            ...
                            nonlocal cind
                            ok = True
                            cind = conditions.currentIndex()
                            dia.close()
                        
                        bttn = QtWidgets.QPushButton("OK ", dia)
                        bttn.move(115, 60)
                        bttn.clicked.connect(pushok)

                        dia.exec()
                        if not ok: return
                        # repcon = logic.conditions[logic.find("c", strat["cond"][cind][1])] # condition to be replaced
                        repcon = strat["cond"][cind]
                        for c in newstrat["conds"]:
                            if c == repcon:
                                pass
                    newstrat["name"] = "M. " + newstrat["name"]
                    comstrat = newstrat # generate strategy to compare against
                elif thing[0] == "s":
                    stind = logic.find("s", thing[1])
                    comstrat = logic.strategies[stind]

            if strats.count() == 0: 
                self.errormsg("No strategy selected.")
                return
            if self.prefs["Calculate strategies on live data"]:
                try:
                    global sp500
                    ...
                    result = QtWidgets.QMessageBox.question(self, "No Internet", "Live data could not be found.\nCalculate strategy on offline data?", 
                    QtWidgets.QMessageBox.StandardButton.Yes | QtWidgets.QMessageBox.StandardButton.No, QtWidgets.QMessageBox.StandardButton.No)
                    if result != QtWidgets.QMessageBox.StandardButton.Yes: # only continue if yes is pressed
                        return
                    self.prefs["Calculate strategies on live data"] = False
            running = True
            procManager.register("review")
            stopbtn.setEnabled(True)
            exbtn.setEnabled(True)
            proc = BackThread(logic.backthread, 1, len(raw), deepcopy(strat["conds"]), 0) # make new process
            proc.calc = deepcopy(strat["calc"])
            proc.risk = deepcopy(strat["risk"])
            if last[0] == ("s", strat["ID"]): proc.rawpoint = processed # to allow it to continue from the same stock when stopped
            else: 
                oList.clear()
                processed = 0
                smart = SmartStats()
                ...
                nonlocal sorts
                sorts = deepcopy(defaultsorts)
                if self.prefs["Calculate strategies on live data"]: oList.attrSort = ["Ticker", "Entry Time", "Exit Percentage"]
                else: 
                    oList.attrSort = ["Ticker", "Exit Spot", "Exit Percentage"]
                    view.isLive = False
            smart.names = [strats.currentText()]
            if comstrat is not None: smart.names.append(combox.currentText())
            if self.prefs["Calculate strategies on live data"]:
                proc.prefs = deepcopy(strat["prefs"])
                if strat["prefs"][2] != "": # get number of stocks to calculate 
                    if proc.prefs[2].count("/") > proc.prefs[2].count("\\"): 
                        count = proc.prefs[2].count("/") # a filepath should contain at least one / or \
                    else: count = proc.prefs[2].count("\\")
                    if count >= 1: isFile = True
                    else: isFile = False
                    if isFile:
                        try:
                            with open(proc.prefs[2]) as file:
                                lines = file.readlines()
                            # preprocess strings
                            final = []
                            for line in lines:
            ...
            proc.queue = queue # set queue to main queue
            proc.doReview = True
            if comstrat is not None:
                proc.revind = 1 # set (1-based) index of strategy
            ind = procManager.indexOf("review")
            procManager.processes[ind].processes.append(proc)
            procManager.processes[ind].processes[-1].start()

            if comstrat is not None:
                numstocks *= 2 # if another strategy is run as well, double number of stocks needed to be calculated
                hassorted[1] = False
                proc = BackThread(logic.backthread, 1, len(raw), deepcopy(comstrat["conds"]), 0) # make new process
                proc.calc = deepcopy(comstrat["calc"])
                proc.risk = deepcopy(comstrat["risk"])
                proc.revind = 2
                # if last[0] == ("s", comstrat["ID"]): logic.backthreads[-1].rawpoint = processed # to allow it to continue from the same stock when stopped
                # else: 
                #     oList.clear()
                #     processed = 0
                if self.prefs["Calculate strategies on live data"]:
                    proc.prefs = deepcopy(comstrat["prefs"])
                proc.queue = queue # set queue to main queue
                proc.doReview = True
                procManager.processes[ind].processes.append(proc)
                procManager.processes[ind].processes[-1].start()
            else:
                hassorted[1] = None # no strategy to get sorted values of
            
            self.threads = []
            self.threads.append(QtCore.QThread(self))
            self.threads[-1].started.connect(getQueue)
            self.threads[-1].finished.connect(self.threads[-1].quit)
            self.threads[-1].finished.connect(self.threads[-1].deleteLater)
            self.threads[-1].start()

            last = [("s", strat["ID"]), combox.vars[combox.currentIndex()]]
        ...
        def export():
            ...
            if file_path == "":
                return 
            dicts = []
            for o in range(oList.count()):
                dicts.append(oList.item(o).dicct)
            df = pd.DataFrame(dicts)
            df.to_excel(file_path, index=False)

        btn = QtWidgets.QPushButton("Run", dbox)
        btn.setGeometry(175, 26, 73, 23)
        btn.clicked.connect(run)
        stopbtn = QtWidgets.QPushButton("Stop", dbox)
        stopbtn.setGeometry(9, 274, 50, 24)
        stopbtn.clicked.connect(stop)
        stopbtn.setEnabled(False)
        ...
        stopbtn.setStyleSheet(disabled_style)

        exbtn = QtWidgets.QPushButton("Export...", dbox)
        exbtn.setGeometry(65, 274, 50, 24)
        exbtn.clicked.connect(export)
        exbtn.setStyleSheet(disabled_style)
        exbtn.setEnabled(False)

        QtWidgets.QLabel("Compare against (opt.)", dbox).move(420, 10)
        combox = DragBox(dbox) # use dragbox to also keep track of variable ids
        combox.setGeometry(420, 26, 150, 22)
        combox.boxType = "C"
        combox.doDrag = False
        combox.addItem("None")
        combox.vars.append(None) # also add buffer to vars
        for strat in logic.strategies:
            combox.addItem(strat["name"])
            combox.vars.append(("s", strat["ID"]))
        for cond in logic.conditions:
            if len(cond["deps"]) == 0: ct = "ci"
            else: ct = "cc"
            combox.addItem(cond["name"])
            combox.vars.append((ct, cond["ID"]))
        obox = QtWidgets.QComboBox(dbox)
        obox.addItems(["(only affects conditions)", "Append", "Remove (only if used)", "Replace with..."]) # "Append negated", 
        obox.setGeometry(250, 26, 150, 22)
        QtWidgets.QLabel("Ticker", dbox).move(250, 260)
        suggs, links = self.generateSuggestions()
        tline = AutoCompleteLineEdit(dbox, suggs)
        tline.links = links
        tline.setGeometry(250, 275, 50, 22)
        QtWidgets.QLabel("Period", dbox).move(310, 260)
        pbox = QtWidgets.QComboBox(dbox)
        pbox.addItems(["1d", "5d", "1mo", "3mo", "6mo", "1y", "2y", "5y", "10y", "ytd", "max"])
        pbox.setGeometry(310, 275, 55, 22)
        QtWidgets.QLabel("Interval", dbox).move(370, 260)
        ibox = QtWidgets.QComboBox(dbox)
        ibox.addItems(["1m", "2m", "5m", "15m", "30m", "60m", "90m", "1h", "1d", "5d", "1wk", "1mo", "3mo"])
        ibox.setGeometry(370, 275, 55, 22)
        QtWidgets.QLabel("Spot", dbox).move(449, 260)
        sline = QtWidgets.QLineEdit(dbox)
        sline.setGeometry(449, 275, 50, 22)

        def change(inc): # changes spot and shows the new scene
            if not isint(sline.text()):
                return
            sline.setText(str(int(sline.text()) + inc))
            showView()

        btn = QtWidgets.QPushButton(">", dbox)
        btn.setGeometry(499, 274, 16, 25)
        btn.clicked.connect(lambda: change(1))
        btn = QtWidgets.QPushButton("<", dbox)
        btn.setGeometry(431, 274, 16, 25)
        btn.clicked.connect(lambda: change(-1))

        def showView(): # display correct thing in view window
            ticker = tline.text()
            period = pbox.currentText()
            interval = ibox.currentText()
            spot = sline.text()
            # error code
            if isint(spot) == False:
                self.errormsg(spot + " is not a valid number.") 
                return
            spot = int(spot)
            if spot < 0:
                self.errormsg(str(spot) + " is not accepted.\nPositive numbers only!")
                return
            avail1 = ["1m", "2m", "5m", "15m", "30m", "60m", "90m", "1h", "1d", "5d", "1wk", "1mo", "3mo"] # available intervals
            if period == "ytd":
                comps = avail1[:avail1.index("60m")] # make interval range
                if interval in comps:
                    self.errormsg("Interval too small for period.")
                    return
            elif period == "max":
                comps = avail1[:avail1.index("1d")] # make interval range
                ...
                if period[-1] == "d": # day range
                    ...
                    if int(period[:-1]) <= 2: # max 2 years
                        ...
                        if interval in avail1[:avail1.index("1d")]:
                            ...
                            return
            errd = False
            if [ticker, period, interval] != view.stockInfo: # new scene
                if view.isLive: data, dates = stock_data(ticker, period=period, interval=interval)
                else:
                    # load correct index from dataset
                    data = read(ticker)
                    dates = []
                if len(data) != 0 and not (len(data) == 1 and data[0] == "Delisted"):
                    view.stockInfo = [ticker, period, interval]
                    view.candles = data
                    view.dates = dates
                else:
                    errd = True
                    self.errormsg("Ticker not found.")
                    return
            if not errd:
                if spot >= len(view.candles):
                    self.errormsg(str(spot) + " is out of range.")
                    return
                view.spot = spot
                view.initScene()

        btn = QtWidgets.QPushButton("Show", dbox)
        btn.setGeometry(519, 274, 52, 25)
        btn.clicked.connect(showView)

        def showInViewer(): # creates a new tab and close dbox
            if not isint(sline.text()):
                self.errormsg(f"{sline.text()} is not a valid number.")
                return
            if int(sline.text()) < 0:
                self.errormsg("Spot has to be a positive number.")
                return
            red, dat = stock_data(tline.text(), period=pbox.currentText(), interval=ibox.currentText())
            if len(red) == 1:
                if type(red[0]) == str:
                    self.errormsg(tline.text() + " hasn't been found.")
                    return
            elif len(red) == 0:
                self.errormsg("Range too big or ticker not found.")
                return
            if int(sline.text()) >= len(red):
                self.errormsg(f"{sline.text()} is out of range.")
                return
            self.timeaxis = dat
            raw.append(red)
            self.newScene("+", "Live " + tline.text().upper(), tline.text())
            self.marked = len(self.candles)*[None] # create a list of none marked spots
            self.marked[int(sline.text())] = "#40ff7700"
            ind = ((int(sline.text())-self.rangex[0])*self.gridconv[0])/self.gridconv[1] # scroll from time
            if ind < self.view.width(): ind = 0
            else: ind -= self.view.width()
        ...
        btn = QtWidgets.QPushButton("In Viewer", dbox)
        btn.setGeometry(499, 300, 72, 25)
        btn.clicked.connect(showInViewer)

        view = SmallView(dbox)
        view.setGeometry(250, 75, 320, 180)
        view.scene().setSceneRect(0, 0, 310, 170)
        grid = Grid(QtCore.QRectF(-5, -5, 320, 180))
        grid.density = (10, 10)
        view.scene().addItem(grid)
        view.pxSize = (320, 180)
        view.isLive = True

        abox = QtWidgets.QComboBox(dbox)
        abox.move(10, 301)
        abox.addItems(["(advanced...)", "Benchmark comparison", "Strategy analysis"])
        analysis = {"Benchmark":[], "Analysis":[]}

        changeWidget = QtWidgets.QWidget(dbox)
        changeWidget.setGeometry(10, 330, 560, 50)
        lay = QtWidgets.QVBoxLayout()
        changeWidget.setLayout(lay)

        def benchmark(ticker): # runs benchmark on current operations from both strategies and compares them with a ticker
            nonlocal analysis
            multiple = False
            if oList.operations == []:
                self.errormsg("Run a strategy first.")
                return
            for org in oList.operations:
                if "Entry Time" not in list(org.keys()):
                    self.errormsg("This only works on live data.")
                    return
                if "No." in list(org.keys()): 
                    multiple = True
                    if org["No."] == 1:
            ...
            l = [False, False]
            if multiple:
                for o in oList.operations:
                    l[o["No."]-1] = True
            if not (l[0] and l[1]):
                self.errormsg("Multiple strategies detected but no data from both of them.")
                return
            groups = [x.name.upper() for x in self.commons if type(x) == Group]
            ...
            if multiple:
                ...
                for o in oList.operations:
                    if "No." in list(org.keys()): 
                        if o["No."] == 2: break # get second strategy
                bal[2] = o["Balance"]
                start[2] = o["Balance"]
            else:
                bal["Main"] = org["Balance"]
                start["Main"] = org["Balance"]
            ops = deepcopy(oList.operations)
            ...
            if multiple:
                ...
                bal["Main"] = start["Main"] # reset balance
            ...
            for d in dates[fkey]:
                ...
                for key in liquid:
                    liquid[key].append((bal[key]+liqpart[key])/start[key])
            findict = {"Date":dates[fkey]}
            for key in liquid:
                coolkey = "Strategy"
                if isint(key):
                    coolkey = coolkey + " " + str(key) # strategy 1/2
                else: coolkey = key + " " + coolkey # main strategy
        ...
        def advanced(): # change widget to match corresponding abox item
            wid = QtWidgets.QWidget()
            if abox.currentText() == "Benchmark comparison":
                QtWidgets.QLabel("Ticker", wid).move(0, 0)
                tedit = AutoCompleteLineEdit(wid, suggs)
                tedit.links = links
                tedit.setGeometry(0, 15, 50, 22)
                btn = QtWidgets.QPushButton("Run", wid)
                btn.setGeometry(60, 14, 50, 24)
                btn.clicked.connect(lambda: benchmark(tedit.text()))
                if len(analysis["Benchmark"]) != 0:
                    ll = QtWidgets.QListWidget(wid)
                    ll.setGeometry(125, 15, 150, 22)
                    for a in analysis["Benchmark"]:
                        if a != "Date":
                            st = f"{a}: {analysis['Benchmark'][a][-1]-1:2.1%}"
                            ll.addItem(st)
                    def export(): # as xlsx
                        file_path, _ = QtWidgets.QFileDialog.getSaveFileName(None, "Save Data...", "", "Excel 2007-365 (*.xlsx)")
                        if file_path == "":
                            return 
                        df = pd.DataFrame(analysis["Benchmark"])
                        df.to_excel(file_path, index=False)

                    btn = QtWidgets.QPushButton("Export", wid)
                    btn.setGeometry(300, 14, 75, 24)
                    btn.clicked.connect(export)
            elif abox.currentText() == "Strategy analysis":
                pass
            while lay.count(): # delete all widgets currently in use
                w = lay.takeAt(0).widget()
                if w is not None:
            ...
            lay.setContentsMargins(0, 0, 0, 0)
            lay.setSpacing(0)
            changeWidget.setLayout(lay) # sets layout of widget

        abox.currentIndexChanged.connect(advanced)

        def finish():
            # get a list of operations
            # ops = oList.operations
            # # get the run values; what strategies were run last, connect it so that pressing run just continues
            # last
            # # get sorting values
            if last[0] is not None: # only save values if something had been done
                sortdict = {"sorts":sorts, "attrSort":oList.attrSort, "sortby":sortby, "hassorted":hassorted}
                logic.reviewData = {"operations":oList.operations, "last":last, "sort":sortdict}
            stop()

        dbox.finished.connect(finish)

        if len(logic.reviewData) != 0: # if review has already been run
            last = logic.reviewData["last"]
            exists = True
            if last[0] is None: exists = False
            if exists:
                strt = logic.strategies[logic.find("s", last[0][1])]
                for s in range(strats.count()):
                    if strats.itemText(s) == strt["name"]: break
                    if s == len(strats)-1: exists = False
            if exists:
                strats.setCurrentIndex(s)
            # check whether condition/strategy in last[0], last[1] still exist
            oList.operations = logic.reviewData["operations"]
            sorts = logic.reviewData["sort"]["sorts"]
            oList.attrSort = logic.reviewData["sort"]["attrSort"]
            sortby = logic.reviewData["sort"]["sortby"]
            hassorted = logic.reviewData["sort"]["hassorted"]
            oList.sortedList(sortby)
    ...
    def loadStrategy(self): # load strategy
        ...
        for d in duples:
            ...
            usids.append(idd) # append bcs now id's in use
    ...
    def connectDialog(self, item): # dialog for connecting / editing conditions
        ...
        def connectExec():
            ...
            if cboxes[2].currentText() != "Not": # if not only exclude the first one and add new condition
                ...
                if len(logic.conditions[logic.find("c", items[indx[1]].idd)]["deps"]) != 0: 
                    deps.append(("cc", items[indx[1]].idd)) # if condition is dependent; append cc (complex condition)
    ...
    def debugButton(self, idd): # start the debug process without selecting debug first
        ...
        self.debugvar = [True, 0, idd, True]
        self.readstocks("0", "debug", "+") # add debug tab
        logic.calcStrategy(idd) # recalculate after switching
        self.backtest(logic.find("s", idd))
        global sp500 
        if self.prefs["Calculate strategies on live data"]:
            # refresh s&p 500 to not have it desync with subprocess
    ...
    def debugNext(self): # next button in debug mode
        ...
        if n == 0:
            if self.prefs["When debugging skip to next marked stock"]:
                self.debugNext()
                return
    ...
    def peek(self, candle: Candle): # runs the command when a candle is clicked
        ...
        names = ["Balance in $", "Order Type", "Order Value(s)", "$ per Order", "Fees per Trade in $"]
        helpers = []
        for strat in logic.strategies:
            needHelper = False
            i = 0
            for r in strat["risk"][0]:
                if type(r) == str and "%" in r: needHelper = True
                if r is None: needHelper = True
            if needHelper: 
                # {"ID", "vars", "data", “color”, “name”, "filters", "deps"}
                calc = [None, strat["risk"][2]]
                extVars = strat["risk"][1] # take usual variables and extend them by adding the risk management values as well
                for r in strat["risk"][0]:
                    if not r is None: # for any value that is not the exit strategy
                        idd = 0
                        j = 0
                        while j < len(extVars): # check if id is already in use
                            if type(extVars[j]) == VariableEquation and extVars[j].id == idd:
                                idd += 1
                                j = -1 # if id in use go up and restart process
                            j += 1
                        extVars.append(VariableEquation(names[i], "Basic", [r, "*", 1], idd)) # create simple equation to display number correctly
                        calc[1].append(("e", idd))
                    i += 1
                # differenciate between helper and normal condition by giving them the ID -1
                condict = {"ID":-1, "vars":extVars, "data":[], "color":"#ffffff", "name":strat["name"] + " Exit", "filters":[-1, "True"], "deps":[], "calc":calc} 
                helpers.append(condict)
        
        allConds = logic.conditions + helpers # combine both lists to allow the gathering of risk varialble values
        stratLabs = [] # strategy exit labels

        #for cond in logic.conditions:
        for cond in allConds:
            if len(cond["deps"]) == 0: # only indicator conditions
                if len(cond["data"]) == 0:
                    logic.getData(logic.find("c", cond["ID"]))
                varsorted = [[], [], [], []] # v, e, x, i
                for var in cond["vars"]:
                    if type(var) == IndicatorVariable:
                        if var.var == "": varsorted[3].append(var) # indicator
                        else: varsorted[0].append(var)
                        if cond["ID"] == -1: # if risk helper is processed; add support indicator
                            i = 0
                            while i < len(varsorted[3]):
                                if varsorted[3][i].indName == var.indName and varsorted[3][i].args == var.args: break # check whether indicator already exists
                                i += 1
                            if i == len(varsorted[3]): # if indicator wasnt found
                                varsorted[3].append(IndicatorVariable(var.indName, var.args, idd=i)) # add indicator
                    elif type(var) == VariableEquation: varsorted[1].append(var)
                    elif type(var) == VariableExpression: varsorted[2].append(var)
                    #elif type(var) == list: filters = var # filters will be passed into variables
                ...
                if cond["ID"] != -1: # for all non helpers
                    lab = QtWidgets.QLabel(wid)
                    lab.move(2, pos)
                ...
                if cond["ID"] != -1: # for all non helpers
                    ...
                    stratLabs.append(st) # save for later to append next to regular strategy
        
        i = 0
        for strat in logic.strategies:
            # add strategy acitvation via complex conditions
            if len(strat["data"]) == 0:
                logic.calcStrategy(strat["ID"]) # first get data of all complex conditions + strategy
            lab = QtWidgets.QLabel(wid)
            lab.move(2, pos)
            if strat["data"][time]: lab.setStyleSheet("border: none; color: #00ff00;")
            else: lab.setStyleSheet("border: none; color: #ff0000;")
            st = strat["name"] + ": " + str(strat["data"][time]) + "("
            for cond in strat["conds"]:
                cind = logic.find("c", cond[1])
                st += logic.conditions[cind]["name"] + ": " + str(logic.conditions[cind]["data"][time]) + ", "
            st = st[:-2] + ")"
            lab.setText(st)
            pos += 20
            if i < len(stratLabs): # add the exit strategy of the same strategy at the end of the strategy
                lab = QtWidgets.QLabel(wid)
                if stratLabs[i].split(": ")[1][0] == "T": lab.setStyleSheet("border: none; color: #00ff00;")
                else: lab.setStyleSheet("border: none; color: #ff0000;")
                lab.move(2, pos)
                lab.setText(stratLabs[i])
                pos += 20
    ...
    def resetWindows(self): # reset docker windows to original state
        for i in range(1, len(self.docks)): # all except first
            ...
            if i == 2 and self.mode.currentText() == "Conditions/Indicators":
                ...
                def markCondition(idd): 
                    if len(logic.conditions[logic.find("c", idd)]["data"]) == 0:
                        logic.getConditionData(conid=idd)
                    self.marked = []
                    for d in logic.conditions[logic.find("c", idd)]["data"]:
                        if d: self.marked.append("#40ff7700")
                        else: self.marked.append(None)
                    self.setScene()
                ...
                for cond in logic.conditions: 
                    if not ("c", cond["ID"]) in alr and len(cond["deps"]) == 0: # if button hasn't already been made or condition is complex
                        ...
                        btn.setClickFn(self.ctrlButton)
                        btn.setRenFn(self.renameButton)
                        btn.setExFn1(markCondition, "Mark")
                ...
                for strat in logic.strategies: 
                    if not strat["ID"] in alr: # if button has already been made
                        ...
                        if theme == "light": btn.setStyleSheet("color:#000000;")
                        btn.setText(strat["name"])
                        btn.setDelFn(self.deleteButton)
                        btn.setToolTip(strat["name"])
                        btn.setClickFn(self.ctrlButton)
                        btn.setRenFn(self.renameButton)
                        btn.setExFn2(self.debugButton, "Debug")
                        btn.setExFn1(self.calcStrategy, "Run")
    ...
    def updateStats(self, ec=0): # update stats periodically until backthreads stop
        ...
        if not self.queue.empty():
            ...
            logic.manyStats[acc[1]] = deepcopy(acc[0][0])
            combined = None
            for s in logic.manyStats:
                if combined is None: combined = deepcopy(s)
                elif type(s) == Stats and type(combined) == Stats:
                    combined.succ += s.succ
                    combined.sf += s.sf
                    combined.processed += s.processed
                    combined.details += s.details
            logic.stats = deepcopy(combined)
    ...
    def reinitIndicators(self): # regather data for the indicators after e.g. the scene switched
        ...
        for sta in logic.strategies:
            sta["data"] = []
            logic.calcStrategy(sta["ID"])
