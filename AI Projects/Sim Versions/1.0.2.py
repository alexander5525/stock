...
for st in usedstocks:
    ...
    maxx = max(pres[usedstocks.index(st)][3])
...
for g in range(gens):
    ...
    for player in players:
        ...
        for stock in usedstocks:
            ...
            print("\033[A                                                   \033[A")
            print(player.fraction[0]/(player.fraction[0]+player.fraction[1]+1))
        # evaluation
        ...
        elif player.fraction[1] > 0: temp = 1 # if no failures are present and at least 1 success then 100% success
        else: temp = 0 # if neither are present then 0% success
        scores.append((players.index(player), temp, player.score, player.average, temp*player.average)) # (index, succ. rate, score, gains in $)
    scores = sorted(scores, key=lambda x: x[4])
    scores.reverse()
    print("\nHighscore: " + str(round(scores[0][2], 2)) + " Success rate: " + str(scores[0][1]) + " Gains: " + str(round(scores[0][3], 2)) + "$ Overall: " + 
    str(round(scores[0][1]*scores[0][3], 2)) + "$")
...
file.write("Highscore,Gain,Success\n")
