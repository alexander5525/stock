...
class Player(): # player that will do the buying
    def __init__(self, is_rand=True, cellnum= 1, readstr=""):
        ...
        if is_rand: # generate random cells
            ...
            self.outs[1] = float(split[7])
            self.outws[1] = float(split[8])
            self.outs[2] = float(split[9])
            self.outws[2] = float(split[10])
            ...
            for sp in split:
                if len(sp) > 0: # so no empty cells exist
                    ...
                    self.cells[-1].weight = float(tings[3])
                    if len(tings) > 4:
                        if len(tings[4]) >= 1: self.cells[-1].exarg[0] = float(tings[4])
                    if len(tings) > 5:
                        if len(tings[5]) >= 1: self.cells[-1].exarg[1] = float(tings[5])
                    if len(tings) > 6:
                        if len(tings[6]) >= 1: self.cells[-1].reverse = bool(int(tings[6]))
...
for g in range(gens):
    ...
    for player in players:
        ...
        for stock in usedstocks:
            ...
            print("\033[A                                                   \033[A")
            print(player.fraction[0]/(player.fraction[0]+player.fraction[1]+0.000000001)) # 0.0000000001 so won't divide by 0
        # evaluation
        ...
        scores.append((players.index(player), temp*player.average, player.score, player.average, temp)) 
        # (index, succ*gains, score, gains in $, succ. rate)
    ...
    for i in templist:
        scores.pop(i)
    print("\nGains: " + str(round(scores[0][3], 2)) + "$ Success rate: " + str(scores[0][4]) + " Score: " + str(round(scores[0][2], 2)) + " W.Average: " + 
    ...
    if g < gens-1: # only mutate if there is a next generation
        ...
        if (g+1) % batchn == 0 and g != 0: # if n batches have happened
...
file.write("Gain,Score,Success\n")
for h in range(len(hiscores)):
    file.write(str(gains[h]) + "," + str(hiscores[h]) + "," + str(succs[h]) + "\n")
file.close()
