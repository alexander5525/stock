...
while runs < 100:
    ...
    if len(raw[-1]) >= 2000: # has to be certain length of timeframes
        ...
        raw.pop(-1)
del got, runs, rn, stocks, read # delete unused variables
...
def condition(index, shape, spot, ma=200):
    ...
    if shape == "up" or shape == "green": # close > open
        ...
        if ma in preinds[0]: # if data was precalculated
        ...
        if spot <= ma: return False
        if ma in preinds[1]: # if precalculated
        ...
        if index in preinds[2]:
            ...
            extremes = get_cont_extrs(stock[spot-1]) # get extremes until spot
        nbox = []
...
class Cell():
    ...
    def calculate(self, st):
        ...
        if time + self.spot < 0: # if spot requested is outside of range
            ...
            self.active = condition(st, self.condition, time+self.spot, self.ma)
...
for i in range(numsts): # for each used stock
    pres.append([])
    for j in range(len(comps)): # complex conditions: ["movavg", "expavg", "contested"]
        pres[-1].append([])
...
for g in range(gens):
    ...
    for player in players:
        ...
        scores.append((players.index(player), player.average))
    scores = sorted(scores, key=lambda x: x[1])
    scores.reverse()
    print("\nHighscore: " + str(scores[0][1]) + "\nMidscore: " + str(scores[len(players)//2][1]))
    ...
    if g < gens-2: # only mutate if there is a next generation
        ...
        for p in players: # reset player averages
            p.reset()
...
file.write("Highscore,Midscore\n")
for h in range(len(hiscores)):
    file.write(str(hiscores[h]) + "," + str(miscores[h]) + "\n")
file.close()
